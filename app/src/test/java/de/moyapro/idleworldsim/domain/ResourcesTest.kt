...
internal class ResourcesTest {
    ...
    fun plusSingle() {
        assertThat(
            Resources(listOf(Resource(Water, 1.0)))
                    + Resources(listOf(Resource(Water, 2.1)))
        ).isEqualTo(
            Resources(listOf(Resource(Water, 3.1)))
        )
    }
    ...
    fun singleConstructor() {
        ...
        val testResources = Resources(listOf(Resource(Minerals, setQuantity)))
    ...
    fun minusSingle() {
        assertThat(
            Resources(listOf(Resource(Water, 10.0)))
                    - Resources(listOf(Resource(Water, 5.1)))
        ).isEqualTo(
            Resources(listOf(Resource(Water, 4.9)))
        )
    }
    ...
    fun resourcesCannotBeNegative() {
        assertThatExceptionOfType(IllegalStateException::class.java)
            .isThrownBy { emptyResources() - Resources(listOf(Resource(Water, 5.1))) }
    }
    ...
    fun hashCodeNotEquals() {
        val resources1 = Resources(listOf(Resource(Minerals, 99.0)))
        val resources2 = Resources(listOf(Resource(Water, 2.0)))
